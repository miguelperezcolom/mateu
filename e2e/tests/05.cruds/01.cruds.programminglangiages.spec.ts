...
test('programming languages crud work', async ({ page }) => {
  await page.goto('/')

  // Expect a title "to contain" a substring.
  await page.getByRole('menuitem', { name: 'Cruds' }).click()
  await page.getByRole('option', { name: 'Programming languages', exact: true }).click()
  await expect(page.locator('h2', { hasText: 'Programming languages' })).toBeVisible()

  // check initial state when loaded
  ...
  await expect(page.getByTestId('filter-main-name').getByPlaceholder('Name')).toBeVisible()
  ...
  await page.getByTestId('filter-main-name').getByPlaceholder('Name').fill('javascript')
  ...
  await page.getByTestId('filter-main-name').getByPlaceholder('Name').fill('c++')
  await page.getByTestId('search').click()
  await expect(page.locator('.lds-roller')).toBeVisible({visible: false})
  ...
  await page.getByTestId('filter-main-name').getByPlaceholder('Name').fill('java')
  await page.getByTestId('search').click()
  await expect(page.locator('.lds-roller')).toBeVisible({visible: false})
  ...
  await page.getByTestId('filter-main-name').getByPlaceholder('Name').fill('x')
  await page.getByTestId('search').click()
  await expect(page.locator('.lds-roller')).toBeVisible({visible: false})
  ...
  await page.getByTestId('filter-main-name').getByPlaceholder('Name').fill('y')
  await page.getByTestId('search').click()
  await expect(page.locator('.lds-roller')).toBeVisible({visible: false})
