...
test.describe('@App(route) — the single app annotation (Phase 5)', () => {
  ...
  test('an app declared with only @App(route) is served and renders as an app', async ({ page }) => {
    ...
    // /appdemo, and the app mounts its distinct home Screen (/appdemo/screen) in the content slot.
    ...
    await expect(page.getByRole('heading', { name: 'App route home screen' })).toBeVisible({ timeout: 15000 });
    await expect(page.getByRole('textbox').first()).toHaveValue('App route home content');
  });
});
