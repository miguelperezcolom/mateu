describe('basic fields', () => {
    beforeEach(() => {
        ...
        cy.get('.v-menubar-menuitem').contains('Url field form').should('be.visible')

        cy.get('.v-menubar-menuitem').contains('Url field form').click();

        cy.get('.viewTitle').contains('Url field form').should('be.visible')
    })
    ...
    it('fields and actions should work as expected', () => {
        ...
        cy.get('.test-assessment').contains('url is null').should('exist')

        cy.get('.test-text').type('dedede');
