describe('basic fields', () => {
    beforeEach(() => {
        ...
        cy.get('.v-menubar-menuitem').contains('Callable field form').should('be.visible')

        cy.get('.v-menubar-menuitem').contains('Callable field form').click();

        cy.get('.viewTitle').contains('Callable field form').should('be.visible')
    })
    ...
    it('fields and actions should work as expected', () => {
        ...
        cy.get('.test-callable').contains('Callable').should('exist')
        cy.get('.test-callable').focus();
        cy.get('.test-callable').click();

        cy.get('.v-Notification').contains('runned for Mateu,14').should('exist')
