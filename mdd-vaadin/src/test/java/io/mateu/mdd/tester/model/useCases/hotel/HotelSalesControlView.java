...
public class HotelSalesControlView extends VerticalLayout {
    ...
    private CheckBoxGroup<String> ver2;
    private Slider minVal;
    private Slider maxVal;

    public HotelSalesControlView() {
        addStyleName(CSS.NOPADDING);

        setSizeFull();

        addComponent(crearFiltros());

        g = crearTreeGrid();
        HorizontalLayout hl = mhl = new HorizontalLayout(g, mlp = new Panel());
        mlp.setWidth("400px");
    ...
    private Component crearFiltros() {
        ...
        vl.addComponent(new ComboBox<>("Hotel", List.of("Aloha", "Valparaiso", "Bellver", "Alcudiamar")));
        vl.addComponent(filtro = new TextField("Filtro"));
        hl.addComponent(vl);
        filtro.addValueChangeListener(e -> g.getDataProvider().refreshAll());

        vl = new VerticalLayout();
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(new ComboBox<>("Agencia", List.of("Logitravel", "Booking.com", "Expedia")));
        vl.addComponent(new ComboBox<>("Mes", List.of("Septiembre 2019", "Octubre 2019", "Noviembre 2019", "Diciembre 2019", "Enero 2020", "Febrero 2020", "Marzo 2020")));
        hl.addComponent(vl);

        vl = new VerticalLayout();
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(new ComboBox<>("Grupo agencias", List.of("Nacional", "Británico", "Alemán")));
        vl.addComponent(new ComboBox<>("Entrada", List.of("Mismo día", "Lunes", "Martes", "Miércoles", "Jueves", "Sábado", "Domingo")));
        hl.addComponent(vl);

        vl = new VerticalLayout();
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(new Slider("Estancia", 1, 30));
        vl.addComponent(new Slider("Release", 0, 30));
        hl.addComponent(vl);


        vl = new VerticalLayout();
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(ver = new CheckBoxGroup<>("Ver", List.of("Cupo", "Precio base", "Ofertas", "Paros", "Ocupaciones")));
        hl.addComponent(vl);
        ver.setValue(Set.of("Ocupaciones"));
        ver.addValueChangeListener(e -> g.getDataProvider().refreshAll());

        vl = new VerticalLayout();
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(ver2 = new CheckBoxGroup<>("Ver", List.of("Release", "Estancia mínima")));
        hl.addComponent(vl);
        ver2.addValueChangeListener(e -> g.getDataProvider().refreshAll());
        
        hl.addComponent(minVal = new Slider(0, 500));
        ...
        hl.addComponent(maxVal = new Slider(0, 500));
        ...
        vl = new VerticalLayout();
        vl.setCaption("Acciones");
        vl.addStyleName(CSS.NOPADDING);
        vl.addComponent(new Button("Abrir/Cerrar", e -> {
            DialogoAbrirCerrar sub = new DialogoAbrirCerrar();

            // Add it to the root component
            UI.getCurrent().addWindow(sub);
        }));
        vl.addComponent(new Button("Crear contrato", e -> {
            DialogoCrearContrato sub = new DialogoCrearContrato();

            // Add it to the root component
            UI.getCurrent().addWindow(sub);
        }));
        vl.addComponent(new Button("Crear oferta", e -> {
    ...
    public class Fila {
        ...
        public Fila(String concepto, String ver, List<Fila> hijos) {
    ...
    private Grid<Fila> crearTreeGrid() {
        ...
        if (false) {
            ...
            dataProvider.setFilter(e -> {
                boolean ok = true;
                ok &= Strings.isNullOrEmpty(filtro.getValue()) || (e.getConcepto() != null && e.getConcepto().toLowerCase().contains(filtro.getValue().toLowerCase()));
            ...
            dataProvider.setFilter(e -> {
                ...
                if (ok) {
                    ok = false;
                    if (ver.getValue().contains("Cupo")) ok |= ((e.getVer() != null && e.getVer().toLowerCase().contains("cupo") || (e.getVer() != null && e.getVer().toLowerCase().contains("ocupación"))));
                    if (ver.getValue().contains("Precio base")) ok |= (e.getVer() != null && e.getVer().toLowerCase().contains("precio base"));
                    if (ver.getValue().contains("Ocupaciones")) ok |= (e.getVer() != null && e.getVer().toLowerCase().contains("precio final"));
                    if (ver2.getValue().contains("Release")) ok |= (e.getConcepto() != null && e.getConcepto().toLowerCase().contains("release"));
                    if (ver2.getValue().contains("Estancia mínima")) ok |= (e.getConcepto() != null && e.getConcepto().toLowerCase().contains("est. mín."));
                }
                e.setVisible(ok);
                return ok;
            });
        ...
        StyleGenerator<Fila> csg = new StyleGenerator<Fila>() {
            ...
            public String apply(Fila fila) {
                ...
                if (ver.getValue().contains("Ofertas")) if (r.nextInt(100) % 4 == 0) s += " " + CSS.YELLOWBGD;
                if (ver.getValue().contains("Paros")) if (r.nextInt(100) % 3 == 0) s += " " + CSS.CANCELLED;
    ...
    private List<Fila> getDatos() {
        ...
        hoteles.add(new Fila("Aloha", "% ocupación", crearHabs()));
        return hoteles;
    }
    ...
    private List<Fila> crearHabs() {
        ...
        for (String x : List.of("Doble", "Suite")) {
            habs.add(new Fila(x, "Cupo total", crearCupos()));
        }
        return habs;
    }
    ...
    private List<Fila> crearCupos() {
        ...
        cupos.add(new Fila("Online", "Cupo", crearContratos()));
        cupos.add(new Fila("Garantía TUI", "Cupo", crearContratosTUI()));
        return cupos;
    }
    ...
    private List<Fila> crearContratos() {
        ...
        for (String x : List.of("Web", "TUI", "Booking/Expedia")) {
            contratos.add(new Fila(x, "Cupo contrato", crearRates()));
        }
        return contratos;
    }
    ...
    private List<Fila> crearRates() {
        List<Fila> cupos = new ArrayList<>();
        cupos.add(new Fila("Release", "Noches", new ArrayList<>()));
        cupos.add(new Fila("Est. mín.", "Noches", new ArrayList<>()));
        for (String x : List.of("SA", "MP", "PC")) {
            cupos.add(new Fila(x, "Precio base", crearCombinaciones()));
        }
        return cupos;
    }
    ...
    private List<Fila> crearCombinaciones() {
        ...
        for (String x : List.of("1+0", "2+0", "2+1", "3+0")) {
            combs.add(new Fila(x, "Precio final", new ArrayList<>()));
        }
        return combs;
    }
