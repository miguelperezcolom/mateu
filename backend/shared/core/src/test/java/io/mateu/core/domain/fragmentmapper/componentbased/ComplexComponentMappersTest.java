...
class ComplexComponentMappersTest {
  ...
  // --- fluent.Page ---
  @Test
  void fluentPageEmpty() {
    assertThat(map(Page.builder().build())).isNotNull();
  }

  @Test
  void fluentPageWithContent() {
    assertThat(
            map(
                Page.builder()
                    .pageTitle("Title")
  ...
  @Test
  void fluentPageWithBreadcrumbs() {
    assertThat(
            map(
                Page.builder()
                    .breadcrumbs(
                        List.of(new Breadcrumb("Home", "/"), new Breadcrumb("Items", "/items")))
                    .build()))
        .isNotNull();
  }

  @Test
  void fluentPageWithAvatar() {
    assertThat(map(Page.builder().avatar(Icon.builder().icon(IconKey.Abacus).build()).build()))
        .isNotNull();
  }

  @Test
  void fluentPageWithBadgesAndKpis() {
    assertThat(
            map(
                Page.builder()
  ...
  // --- fluent.Form ---
  @Test
  void fluentFormEmpty() {
    assertThat(map(Form.builder().build())).isNotNull();
  }

  @Test
  void fluentFormWithContent() {
    assertThat(
            map(
                Form.builder()
                    .title("Form")
    ...
    assertThat(
            map(
                FormField.builder()
                    ...
                    .dataType(FieldDataType.string)
                    .options(
                        List.of(new Option("opt1", "Option 1"), new Option("opt2", "Option 2")))
                    .build()))
        .isNotNull();
  }
  ...
  @Test
  void formFieldDate() {
    assertThat(map(FormField.builder().id("date").dataType(FieldDataType.date).build()))
        .isNotNull();
  }

  @Test
  void formFieldBoolean() {
    assertThat(map(FormField.builder().id("active").dataType(FieldDataType.bool).build()))
        .isNotNull();
  }

  @Test
  void formFieldInteger() {
    assertThat(map(FormField.builder().id("count").dataType(FieldDataType.integer).build()))
        .isNotNull();
  }

  @Test
  void formFieldExternalFile() {
    assertThat(
            map(
                FormField.builder()
                    .id("file")
  ...
  @Test
  void directory() {
    assertThat(map(Directory.builder().menu(List.of()).build())).isNotNull();
  }
  ...
  @Test
  void gridGroupColumn() {
    assertThat(
            map(
                GridGroupColumn.builder()
                    .label("Group")
