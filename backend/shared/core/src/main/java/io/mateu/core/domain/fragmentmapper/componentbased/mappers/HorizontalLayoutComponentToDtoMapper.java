...
import io.mateu.dtos.HorizontalLayoutJustificationDto;
import io.mateu.dtos.HorizontalLayoutVerticalAlignmentDto;
...
public class HorizontalLayoutComponentToDtoMapper {
  ...
  public static ClientSideComponentDto mapHorizontalLayoutToDto(
    ...
    var metadataDto = HorizontalLayoutDto.builder()
            ...
            .spacingVariant(horizontalLayout.spacingVariant() != null?
                    SpacingVariantDto.valueOf(horizontalLayout.spacingVariant().name()):null)
            .justification(horizontalLayout.justification() != null?
            HorizontalLayoutJustificationDto.valueOf(horizontalLayout.justification().name()):null)
            .flexGrows(horizontalLayout.flexGrows())
            .verticalAlignment(horizontalLayout.verticalAlignment() != null?
                    HorizontalLayoutVerticalAlignmentDto.valueOf(horizontalLayout.verticalAlignment().name()):null)
            .wrap(horizontalLayout.wrap())
            .build();
    return new ClientSideComponentDto(
        metadataDto,
        horizontalLayout.id(),
        applyFlowGrows(horizontalLayout.content().stream()
            .map(content -> mapComponentToDto(null, content, baseUrl, route, httpRequest))
            .toList(), horizontalLayout.flexGrows()),
        horizontalLayout.style(),
        horizontalLayout.cssClasses());
  }

  private static List<ComponentDto> applyFlowGrows(List<ComponentDto> components, List<Integer> flexGrows) {
