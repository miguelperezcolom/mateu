  ...
  public Flux<?> run(Object instance, RunActionCommand command) {
    ...
    if (actionId.startsWith("codesearch-")) {
      ...
      return Flux.just(Dialog.builder()
                      .content(wrapWithServerSideComponent(createComponent(optionsSupplier, command.baseUrl(), command.route(), command.consumedRoute(), command.initiatorComponentId(), command.httpRequest()),
                              optionsSupplier,
                              command.baseUrl(),
                              command.route(),
    ...
    private Component wrapWithServerSideComponent(Component clientSideComponent,
