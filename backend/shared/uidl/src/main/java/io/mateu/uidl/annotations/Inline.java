 ...
 *       such as {@code AutoEditableView}): the inner view drops badges/kpis, demotes its title
 *       from {@code h2} to {@code h3} so it nests under the host, and (for single-section forms)
 *       drops the outlined Card wrapper around its content. The parent {@link Section} that hosts
 *       it also drops its own title row so the embedded {@code h3} title + toolbar render as one
 *       coherent row inside the section's card. For tabs (which have no parent title row), the
 *       embedded {@code h3} becomes the only visible title — remove {@link Title} from the inner
 *       model to suppress it entirely.
 * </ul>
 */
