...
public class ReactorTest {
    ...
    public void contextIsAlsoRead() {
        ...
        StepVerifier.create(Mono.empty().then(Mono.just("hols")))
                .expectNext("hols")
                .verifyComplete();
    ...
    public void contextIsReadAgain() {
        var mono = Mono.deferContextual(ctx ->
                                Mono.just("Hello " + ctx.get("name")));
        mono = mono.log();
