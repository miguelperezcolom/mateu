...
public class FieldInterfacedFromRemoteField implements FieldInterfaced {
      ...
      choice =
          new Choice() {
            ...
            public List<Option> getOptions() {
              return f.getAttributes().stream()
                  .filter(p -> "choice".equals(p.getKey()))
  ...
  public boolean isAnnotationPresent(Class<? extends Annotation> annotationClass) {
    if (NotNull.class.equals(annotationClass)
        || NotEmpty.class.equals(annotationClass)
        || NotBlank.class.equals(annotationClass)) {
      return f.getValidations() != null
          && f.getValidations().stream()
              .filter(v -> ValidationType.NotEmpty.equals(v.getType()))
              .findAny()
              .isPresent();
    }
