...
public class Helper extends SlimHelper {
  ...
  public static boolean intersects(
      LocalDate start, LocalDate end, LocalDate checkIn, LocalDate checkOut) {
    return (start == null || start.isBefore(checkOut))
        && (end == null || end.compareTo(checkIn) >= 0);
  }

  public static boolean cabe(
      LocalDate validFrom, LocalDate validTo, LocalDate checkIn, LocalDate checkOut) {
  ...
  public static String httpPost(String url, Map<String, String> parameters) throws IOException {
    ...
    HttpRequest request =
        requestFactory.buildPostRequest(new GenericUrl(url), new UrlEncodedContent(""));
    return request.execute().parseAsString(); // .parseAs(VideoFeed.class);
  }
  ...
  public static String runCommand(String command) throws IOException, InterruptedException {
    ...
    Process process =
        Runtime.getRuntime()
            .exec(System.getProperty("os.name").toLowerCase().contains("win") ? wcmd : cmd);
  ...
  public static String eval(String engineName, String operations, Map<String, Object> params) {
    ...
    // log.info("engine.eval('" + operations + "') instantiating engine took " +
    ...
    // log.info("engine.eval('" + operations + "') took " + (System.currentTimeMillis() - t1) +
    // "ms");
