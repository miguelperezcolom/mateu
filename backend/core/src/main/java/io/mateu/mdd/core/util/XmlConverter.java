...
public class XmlConverter implements AttributeConverter<Object, String> {
  ...
  public String convertToDatabaseColumn(Object xmlSerializable) {
    if (xmlSerializable == null) return null;
      ...
      Element xml =
          (xmlSerializable instanceof XMLSerializable)
              ? ((XMLSerializable) xmlSerializable).toXml()
              : Serializer.toXml(xmlSerializable);
      if (xml == null) return null;
      else {
        if (xml.getAttribute("className") == null)
          xml.setAttribute("className", xmlSerializable.getClass().getName());
