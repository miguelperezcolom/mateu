    ...
    private Menu createMenu(MenuEntry menuEntry) {
        ...
        if (menuEntry instanceof AbstractAction) {
            if (menuEntry instanceof MDDOpenEditorAction) {
                ...
                JourneyStoreAccessor.get().putJourneyPerType(menuEntry.getId(),
                        new JourneyMapper().map(action.getBean()), action.getBean());
                ...
                try {
                    ...
                    JourneyStoreAccessor.get().putJourneyPerType(menuEntry.getId(),
                            new JourneyMapper().map(action), viewBuilder.buildView(action));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                try {
                    JourneyStoreAccessor.get().putJourneyPerType(menuEntry.getId(),
                            new JourneyMapper().map(action),
                            ReflectionHelper.newInstance(action.getListViewClass()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
                JourneyStoreAccessor.get().putJourneyPerType(menuEntry.getId(),
                        new JourneyMapper().map(menuEntry), menuEntry);
            }
        }
        return Menu.builder()
