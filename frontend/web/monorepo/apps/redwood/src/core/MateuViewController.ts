...
export interface RenderedView {
  ...
  loading: boolean;
  error: string | null;
  /** Bumped on every (re)render so hosts can key remounts. */
  version: number;
}
...
/**
 * Per-view session/navigation state and the UIIncrement applier — the proven AppContext state
 * machine (JavaFX / IntelliJ / React Native): navigate/runAction → applyIncrement → fragments
 * (component / data-only / state-only navigation) + messages + commands + appState. Pure
 * TypeScript, no view-framework imports — a thin host subscribes through [onRender].
 *
 * Ported from the React Native renderer (frontend/app/react-native/src/core/MateuViewController.ts).
 */
export class MateuViewController {
  ...
  async runAction(actionId: string, parameters?: Json, silent = false): Promise<void> {
    ...
    // Client-side validation: flagged actions and bubbled crud submits.
  ...
  /** Renders a component fragment: ServerSide wrappers unwrap here. */
