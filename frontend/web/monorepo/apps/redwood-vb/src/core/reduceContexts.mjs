...
export function reduceContexts(reg, increment) {
  ...
  for (const fr of increment.fragments || []) {
    ...
    // Contrato real (fixtures/real): un ServerSide lleva route/serverSideType/initialData al
    // NIVEL SUPERIOR del component (no en .metadata); guardarlos permite reconstruir el
    // RunActionRqDto saliente (Fase 1.4) y sembrar el state desde initialData en el load.
    ...
    contexts[id] = {
      ...
      route: md.route ?? prev.route,
      serverSideType: md.serverSideType ?? prev.serverSideType,
      consumedRoute: md.consumedRoute ?? prev.consumedRoute,
      pageType: md.pageType ?? prev.pageType,
      pageWidth: md.pageWidth ?? prev.pageWidth,
      state:
        fr.action === 'ReplaceKeepData'
          ? { ...prev.state, ...(fr.state || {}) }
          : increment.componentState ||
            md.initialData ||
            fr.state?.[fr.component?.id] ||
            fr.data ||
            prev.state,
      dirty: false,
    }
