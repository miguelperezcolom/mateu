...
export const renderGrid = (component: ClientSideComponent, data: any) => {
    ...
    if (metadata.tree) {
        ...
        >
            ...
            <vaadin-grid-column path="${column.id}">${index} - ${(column.metadata as GridColumn)?.label}</vaadin-grid-column>
`:html`
...
`)}
    ...
    return html`
        ...
        >
            ...
            <vaadin-grid-column path="${column.id}">${(column.metadata as GridColumn)?.label}</vaadin-grid-column>
`)}
        </vaadin-grid>
    `
