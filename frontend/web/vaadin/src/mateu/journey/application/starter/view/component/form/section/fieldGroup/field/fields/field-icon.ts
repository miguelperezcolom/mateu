...
@customElement('field-icon')
export class FieldIcon extends LitElement implements Component {
    ...
    protected update(changedProperties: PropertyValues) {
        ...
        this.src = this.field?.attributes?.find(a => a.key == 'icon-src')?.value as string
        if (this.src) {
            this.svg = svg`${unsafeHTML(this.value)}`
        }
    ...
    render() {
        return html`
            ...
            <vaadin-icon icon="${ifDefined(this.src?undefined:this.value)}" 
                         size="1024" 
                         .svg="${ifDefined(this.src?this.svg:undefined)}" 
                         data-testid="${this.name}" 
                         style="${this.styles}" 
                         class="${this.cssClasses} ${this.action?'clickable':''}"
