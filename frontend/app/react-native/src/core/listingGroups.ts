/**
 * LISTING AGGREGATES + ROW GROUPING — the React Native port of the web renderers' shared
 * helpers (frontend/web/monorepo/libs/mateu/src/mateu/ui/infra/ui/listingGroups.ts). Keep the
 * walk and formatting rules IDENTICAL to the reference so every renderer shows the same values.
 *
 * The wire contract: crud metadata carries `groupBy` (the row field id the listing is grouped
 * by; rows arrive pre-sorted so groups are contiguous) and grid columns may carry `aggregate`
 * ('sum'|'avg'|'min'|'max'|'count'); the search result envelope (the same object that carries
 * `page`) carries `aggregates` (whole-filtered-set totals per aggregated column) and `groups`
 * (per-group summaries, in listing order).
 */

/**
 * Per-group summary the server sends next to the page when the listing is grouped: one entry
 * per group, in listing order, with the aggregates of each aggregated column over the WHOLE
 * filtered set of that group (not just the current page).
 */
...
/** The listing/search result envelope the crud stores as its rows source. */
...
/** The minimal column shape the aggregate helpers need. */
...
export interface GroupRow {
  ...
  /** Synthetic row id, distinct from any data row's. */
  _rowNumber: string;
}
 ...
 * `groupBy` field changes. Markers are per-page and computed client-side. Group summaries are
 * matched by value (string-compared); a run with no matching summary gets one synthesized from
 * the page rows. No groupBy / no groups → the rows pass through unchanged.
 */
 ...
 * money-cell format (de-DE, 2 decimals); count → integer; anything else → locale number with
 * max 2 decimals.
 */
...
): string | undefined => (groupBy && columnIds.includes(groupBy) ? groupBy : columnIds.find((id) => !!id));
...
/**
 * Footer texts per column id for the totals row pinned at the bottom of the table: each
 * aggregated column shows its formatted whole-filtered-set total; the first column shows
 * "Total" — or `Total (rowCount)` when the first column is the group column. Returns undefined
 * when there is nothing to total, meaning: render no totals row at all.
 */
export const buildAggregateFooters = (
  ...
  if (first && footers[first.id] === undefined) {
    ...
    footers[first.id] =
      groupBy && first.id === groupBy && totalRows != null ? `Total (${totalRows})` : 'Total';
  }
  return footers;
};
