...
/** Jewel port of the shared `PageRenderer`: header (title/subtitle/toolbar), children, bottom buttons. */
@Composable
        ...
        if (headerText.isNotBlank() || subtitle.isNotBlank() || toolbar.isNotEmpty()) {
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                if (headerText.isNotBlank()) {
                    Text(headerText, style = JewelStyle.pageTitle())
                }
                if (subtitle.isNotBlank()) {
                    Text(subtitle, style = JewelStyle.body(), color = JewelStyle.muted)
                }
...
private fun Banner(banner: JsonNode) {
    ...
    Column(
        ...
        banner.text("title").takeIf { it.isNotBlank() }?.let {
            Text(it, color = fg, fontWeight = FontWeight.Bold, style = JewelStyle.subSectionTitle())
        }
        banner.text("description").takeIf { it.isNotBlank() }?.let {
            Text(it, color = fg, style = JewelStyle.body())
        }
    }
}
