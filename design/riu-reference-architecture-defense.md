...
como v0/Copilot/Mentor — no hay que sobrevenderlo.
...
| GAP‑4 | **Autoría IA productizada** (prompt-to-app en IDE + superficie MCP/agente que respeta RBAC) | v0/Copilot (A), Retool/ToolJet AI (C), Mentor/Maia/Copilot (D), Directus MCP (B) | Media | **Investigar**: el AI-drives-UI en runtime es diferenciado, pero falta el loop end-user prompt-to-app y un MCP GA. Como la UI es dato, un **servidor MCP que respeta RBAC + un harness que emite UIDL** son alto-apalancamiento y construibles. La ventaja rival es más estrecha de lo que parece (§6.3.1). |
...
- **R4 · MCP que respeta RBAC + harness que emite UIDL (GAP‑4)** — alto apalancamiento porque la UI es
  dato; la ventaja rival de "IA agéntica GA" está sobrevendida (verificado). Decide si entra en v3 o v3.x.
...
| R4 MCP+UIDL codegen | GAP‑4 | Ventaja rival sobrevendida; alto apalancamiento | No (investigar) |
