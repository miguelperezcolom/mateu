...
- **Dark/light mode toggle**: add `themeToggle = true` to `@App` to show a moon/sun icon button in the header of all app variants. Clicking it toggles `document.documentElement` `theme` attribute between `"dark"` and none (Lumo convention). Preference is persisted in `localStorage` under `mateu-theme` and restored on `connectedCallback`. Implemented via `MateuApp.toggleTheme()` and `renderThemeToggle()` helper in `appRenderer.ts`.
