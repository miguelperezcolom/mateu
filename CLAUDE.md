...
- **Page banners (`@Banner`)**: annotate methods with `@Banner(theme=BannerTheme.INFO, title="...")` to show messages below the page header and above the first form section, rendered as `vaadin-card`. If the method returns `String`, that value is used as the description (evaluated dynamically on each request). Themes: `INFO` (blue), `SUCCESS` (green), `WARNING` (amber), `DANGER` (red). Wire type: `BannerDto` (reuses existing `PageDto.banners` field). Frontend rendering in `mateu-page.ts`.
