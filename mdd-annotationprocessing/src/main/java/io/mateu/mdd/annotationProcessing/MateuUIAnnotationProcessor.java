...
public class MateuUIAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (TypeElement annotation : annotations) {
            ...
            for (Element e : annotatedElements) {
                ...
                try {
                    createUI(pkgName, simpleClassName, className, e);

                    createServlet(generatedFullClassName, pkgName, className, simpleClassName, e,
                            generatedClassName);
    ...
    private void createServlet(String generatedFullClassName, String pkgName, String className,
        ...
        try (PrintWriter out = new PrintWriter(builderFile.openWriter())) {
            ...
            out.println();
            out.println("@WebServlet(urlPatterns = {\"" + e.getAnnotation(MateuUI.class).path() +
                    "\", \"" + ("/".equals(e.getAnnotation(MateuUI.class).path())?"":
                    e.getAnnotation(MateuUI.class).path()) + "/*\"}, name = \"" +
                    className.replaceAll("\\.","_") +
                    "UIServlet\", asyncSupported = true, initParams = {\n" +
                    "        @WebInitParam(name = \"org.atmosphere.websocket.suppressJSR356\", value = \"true\")})");
            out.println("@VaadinServletConfiguration(ui = " + simpleClassName +
                    "UI.class, productionMode = false)");
            out.println("@MateuUIServlet(path=\"" + e.getAnnotation(MateuUI.class).path() + "\")");
            out.println("public class " + generatedClassName +
                    " extends VaadinServlet implements SessionInitListener {");
            out.println("");

            out.println("    @Override");
            out.println("    protected DeploymentConfiguration createDeploymentConfiguration(Properties " +
                    "initParameters) {");
            out.println("        if (\"true\".equals(System.getProperty(\"productionMode\"))) " +
            ...
            out.println("    @Override\n" +
                    ...
                    ("".equals(e.getAnnotation(MateuUI.class).path())?"/":
                            e.getAnnotation(MateuUI.class).path()) + "_app\", " +
    ...
    private void createUI(String pkgName, String simpleClassName, String className, Element e)
            throws IOException {
