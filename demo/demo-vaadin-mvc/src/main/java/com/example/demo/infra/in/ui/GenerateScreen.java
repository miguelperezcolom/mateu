...
/**
 * "Prompt → screen": type what you want, an LLM authors the Mateu definition (YAML), and it renders
 * IN THIS PAGE through the normal pipeline (so it looks like any real Mateu Vaadin screen). The
 * generated tree is held in a {@link Callable} island, exactly like the built-in archetypes. Demo of
 * the authoring half of the agent-operability plane. Set MATEU_AGENT_URL / MATEU_AGENT_TOKEN to point
 * at an LLM agent (a plain-text /chat endpoint that returns the YAML).
 */
@UI("generate")
@Title("Prompt → screen")
public class GenerateScreen {
  ...
  @Button
  Runnable generate = this::doGenerate;
  ...
  private static String author(String prompt) throws Exception {
    ...
    String body = "{\"message\":" + json(instr + prompt) + ",\"sessionId\":\"gen-inapp\"}";
    var builder =
