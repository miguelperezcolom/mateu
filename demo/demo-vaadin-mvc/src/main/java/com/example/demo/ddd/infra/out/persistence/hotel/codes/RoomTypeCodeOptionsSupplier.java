...
public class RoomTypeCodeOptionsSupplier implements ForeignKeyOptionsSupplier {
    ...
    public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
        var found = roomTypeCodeRepository.search(searchText , pageable);
        return new ListingData<>(new Page<>(
                searchText,
                found.page().pageSize(),
                found.page().pageNumber(),
                found.page().totalElements(),
                found.page().content().stream().map(roomTypeCode ->
                        new Option(roomTypeCode.code(), roomTypeCode.name())).toList()));
    }
}
