...
public class FileIdOptionsSupplier implements ForeignKeyOptionsSupplier {

    final FileRepository fileRepository;

    @Override
    public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
        var found = fileRepository.search(searchText , pageable);
        return new ListingData<>(new Page<>(
                searchText,
                found.page().pageSize(),
                found.page().pageNumber(),
                found.page().totalElements(),
                found.page().content().stream().map(file ->
                        new Option(file.id(), file.name())).toList()));
    }
}
