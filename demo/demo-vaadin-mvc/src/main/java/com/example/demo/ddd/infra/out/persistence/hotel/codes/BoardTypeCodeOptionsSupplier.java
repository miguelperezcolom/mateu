...
public class BoardTypeCodeOptionsSupplier implements ForeignKeyOptionsSupplier {
    ...
    public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
        var found = boardTypeCodeRepository.search(searchText , pageable);
        return new ListingData<>(new Page<>(
                searchText,
                found.page().pageSize(),
                found.page().pageNumber(),
                found.page().totalElements(),
                found.page().content().stream().map(boardTypeCode ->
                        new Option(boardTypeCode.code(), boardTypeCode.name())).toList()));
    }
}
