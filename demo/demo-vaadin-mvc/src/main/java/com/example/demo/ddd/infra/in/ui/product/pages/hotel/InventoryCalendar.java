...
public class InventoryCalendar extends GenericForm {
    ...
    @Colspan(8)
    @ReadOnly
    List<InventoryCalendarLine> lines;
   ...
   int rooms;
    ...
    public Object handleAction(String actionId, HttpRequest httpRequest) {
        if ("search".equals(actionId)) {
            ...
            lines = hotelRepository.findById(hotelId).get().inventoryCalendarLines().stream().filter(line -> line.roomTypeCode().equals(roomId)).toList();
        }
        if ("set".equals(actionId)) {
            ...
            lines = hotel.inventoryCalendarLines();
            ...
            lines = openOrClose(lines, from, to, rooms);
            hotelRepository.saveAll(List.of(hotel.withInventoryCalendarLines(lines)));
            lines = lines.stream().filter(line -> line.roomTypeCode().equals(roomId)).toList();
        }
        return super.handleAction(actionId, httpRequest);
    }

    private List<InventoryCalendarLine> openOrClose(List<InventoryCalendarLine> lines, LocalDate from, LocalDate to, int inventory) {
        var result = new ArrayList<InventoryCalendarLine>();
