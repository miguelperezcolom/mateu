package com.example.demo.infra.ui.menus.useCases.processDefinition.main;

import com.example.demo.infra.ui.menus.useCases.processDefinition.main.insprod.InputRepository;
import com.example.demo.infra.ui.menus.useCases.processDefinition.main.insprod.JourneyRepository;
import com.example.demo.infra.ui.menus.useCases.processDefinition.main.insprod.Product;
import com.example.demo.infra.ui.menus.useCases.processDefinition.main.insprod.ProductRepository;
...
public class Updater {
              ...
              i.getValidations()
                  .forEach(
                      v -> {
                        ...
                        if (v.startsWith("dateFormat:"))
                          field.setPattern(v.substring("dateFormat:".length()));
  ...
  private FieldGroup createFieldGroup(Section section) {
    return fieldGroupRepository.save(
        FieldGroup.builder()
            .id(UUID.randomUUID().toString())
            .section(section)
  ...
  private Step createStep(Flow flow) {
    return stepRepository.save(
        Step.builder().id(UUID.randomUUID().toString()).flow(flow).title("Single step").build());
  }
}
