...
/**
 * Check-in v4 — the <b>drawer</b> variant. The page shows only the <b>essential</b> check-in
 * information (reservation overview, check-in data and the guest list), so the common case is fast
 * and uncluttered. All the <b>secondary</b> reference information (empresa, tarjeta, histórico,
 * preferencias, importes, habitación, historial, folios) lives behind a single toolbar button
 * ("Detalle del cliente ▸", {@code ctrl+alt+d}) that slides in a <b>modeless</b> {@link Drawer} from
 * the right — modeless so the receptionist can consult it while still working the main form.
 *
 * <p>Inside the drawer the sections are laid out as <b>tabs</b> ({@link CheckInReferenceDrawer}),
 * each with its own {@code alt+N} shortcut, so any detail is one keystroke away — and only opened
 * when actually needed. This resolves the "where do the triggers go / a right-hand index would be
 * hidden by the drawer" dilemma: the per-section navigation (tabs) lives <i>inside</i> the drawer,
 * always visible while it is open.
 *
 * <p>Contrast with v2 (everything stacked with a sticky TOC) and v1/v3 (master-detail zones).
 */
@Service
@Scope("prototype")
@Route(value = "/checkin/:id/v4", parentRoute = "")
@Style(StyleConstants.FULL_WIDTH_WITH_PADDING)
@ReadOnly
@PlainText
@Title("Check-in")
public class CheckInFormV4 implements HeaderSupplier, TriggersSupplier, Hydratable {

  final ReservationLineRepository repository;

  public CheckInFormV4(ReservationLineRepository repository) {
  ...
  // ── Drawer trigger ────────────────────────────────────────────────────────
  // Opens the tabbed reference panel on demand, modeless so the main form stays usable underneath.
  @Toolbar
  @Action(shortcut = "ctrl+alt+d")
  @Label("Detalle del cliente ▸")
  Drawer openDetail(HttpRequest httpRequest) {
    var id = idFromRoute(httpRequest);
    var panel = new CheckInReferenceDrawer();
    if (id != null) {
      repository.findById(id).ifPresent(panel::populate);
    }
    return Drawer.builder()
        .id("checkin-detail-drawer")
        .headerTitle("Detalle del cliente")
        .position(DrawerPosition.end)
        .width("44rem")
        .modeless(true)
        .content(new ModelViewComponent(panel))
        .build();
  }
    ...
    return repository
        ...
        .map(
            line -> {
              ...
              var foliosForHeader = new FoliosSection();
              foliosForHeader.populate(line);
              var saldo =
                  foliosForHeader.saldoPendiente != null
                      ? foliosForHeader.saldoPendiente.toPlainString()
                      : "0";
              var info =
