package io.mateu.mdd.demoadminpanel.infra.in.ui.generaloverview;
...
/**
 * Demo of the {@link GeneralOverview} archetype (the Redwood "General Overview" template): the
 * requisition context switcher at the top, and the selected record's overview below — the
 * EntityHeader metadata strip over side-by-side property cards.
 */
@UI("/general-overview-demo")
@Title("Requisitions")
public class RequisitionOverview extends GeneralOverview<RequisitionOverview.Requisition> {
        ...
        Card.builder()
            ...
            .content(
                StatusList.builder()
                    .items(
                        req.lines().stream()
                            .map(line -> StatusItem.builder().title(line).status("OK").statusColor("success").build())
                            .toList())
                    .frameless(true)
                    .build())
