...
It is opt-in and non-breaking: pages without `@Compact` are unaffected. The preset is also available directly as `StyleConstants.COMPACT` for use with [`@Style`](/java-ui-definition/annotations/styling/).
