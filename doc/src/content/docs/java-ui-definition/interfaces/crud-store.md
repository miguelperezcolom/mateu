...
1. loads `findAll()`,
2. keeps rows whose text contains **every whitespace-separated word** of `searchText`, case-insensitively and in any order — using `SearchableText.searchableText()` when the entity implements [`SearchableText`](/java-ui-definition/interfaces/searchable-text/), otherwise `toString()`,
